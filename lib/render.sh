    ...
    jq -n \
            ...
            };
        {
            ...
            inbounds: [
                ...
                {
                    tag: (if ($state[0].mode // "") == "xhttp-reality-self" then "VLESS-XHTTP-REALITY-SELF" else "VLESS-XHTTP-REALITY" end),
                    listen: "0.0.0.0",
                    port: ($state[0].reality.listen_port // 443),
                    protocol: "vless",
                    settings: {
...
switch_xhttp_reality_self() {
    local domain="$1" acme_email="$2" address="$3" path="$4" port="$5" fallback_port="$6"
    case "$domain" in
        *@*) die "--domain must be a DNS name, not an email. Use --email for the ACME email." ;;
    esac
    validate_domain "$domain" || die "Invalid domain: $domain"
